...
* https://lager.readthedocs.io
* https://github.com/metachris/lager
...
In order to also log to a file, just use `lager.logfile(..)`:

    lager.logfile("/tmp/test.log")

If you want to use specific loggers instead of the global default logger, use
`setup_logger(..)`:

    logger = lager.setup_logger(logfile="/tmp/test.log")

The default loglevel is `logging.DEBUG`. You can set it with the
parameter `level`.

See the documentation for more information: https://lager.readthedocs.io
"""
import functools
import os
import sys
...
class LagerRecord(logging.LogRecord):
...
class Lager(logging.getLoggerClass()):
    ...
    # @staticmethod
    def mklog(self, *args, **kwargs):
        return LagerRecord(*args, **kwargs)
        # self.log(level, msg)

    def __add__(self, msg):
        curframe = inspect.currentframe()
        framo = inspect.stack()[1]
        ...
        # rec1= logging.LogRecord(name=self.name, level=self.level, pathname=info.filename, lineno=info.lineno, func=info.function, msg=msg, exc_info=(), args=None)
        # print(rec1)
        # rec = self.mklog(name=self.name, level=self.level, pathname=info.filename, lineno=info.lineno, func=info.function, msg=msg, exc_info=(), args=None)
        # print(rec)
        ...
        # rec = logging.LogRecord(name=self.name,
                                # level=self.level,
                                level=logging.INFO,
                                lno=info.lineno,
                                fn=call_fn,
...
def syslog(logger_to_update=logger, facility=SysLogHandler.LOG_USER, disableStderrLogger=True):
    ...
    # Setup lager to only use the syslog handler with the specified facility
